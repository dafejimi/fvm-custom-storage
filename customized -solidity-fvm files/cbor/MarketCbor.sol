...
library MarketCBOR {
    ...
    function serializePublishCustomStorageDealsParams(MarketTypes.PublishStorageDealsParams memory params) internal pure returns (bytes memory) {
        uint256 capacity = 0;

        capacity += Misc.getPrefixSize(1);
        capacity += Misc.getPrefixSize(params.customDeals.length);

        for (uint64 i = 0; i < params.customDeals.length; i++) {
            capacity += Misc.getPrefixSize(2);
            capacity += Misc.getPrefixSize(14);

            capacity += Misc.getCidSize(params.customDeals[i].proposal.piece_cid.data);
            capacity += Misc.getPrefixSize(params.customDeals[i].proposal.piece_size);
            capacity += Misc.getBoolSize();
            capacity += Misc.getBytesSize(params.customDeals[i].proposal.client.data);
            capacity += Misc.getBytesSize(params.customDeals[i].proposal.provider.data);
            capacity += Misc.getBytesSize(params.customDeals[i].proposal.label.data);
            capacity += Misc.getChainEpochSize(params.customDeals[i].proposal.start_epoch);
            capacity += Misc.getChainEpochSize(params.customDeals[i].proposal.end_epoch);
            capacity += Misc.getBytesSize(params.customDeals[i].proposal.storage_price_per_epoch.serializeBigInt());
            capacity += Misc.getBytesSize(params.customDeals[i].proposal.provider_collateral.serializeBigInt());
            capacity += Misc.getBytesSize(params.customDeals[i].proposal.client_collateral.serializeBigInt());
            capacity += Misc.getChainEpochSize(params.customDeals[i].proposal.CustomOptions.termination_epoch);
            capacity += Misc.getPrefixSize(params.customDeals[i].proposal.CustomOptions.share_percentage);
            capacity += Misc.getBoolSize();

            capacity += Misc.getBytesSize(params.deals[i].client_signature);
            
            /*
            (customDealCap, customDealBuf) = serializeCustomOptions(params.customDeals[i].proposal.CustomOptions);
            capacity += customDealCap;
            */
        ...
        buf.startFixedArray(1);
        buf.startFixedArray(uint64(params.customDeals.length));

        for (uint64 i = 0; i < params.customDeals.length; i++) {
            buf.startFixedArray(2);

            buf.startFixedArray(14);

            buf.writeCid(params.customDeals[i].proposal.piece_cid.data);
            buf.writeUInt64(params.customDeals[i].proposal.piece_size);
            buf.writeBool(params.customDeals[i].proposal.verified_deal);
            buf.writeBytes(params.customDeals[i].proposal.client.data);
            buf.writeBytes(params.customDeals[i].proposal.provider.data);
            buf.writeDealLabel(params.customDeals[i].proposal.label);
            buf.writeChainEpoch(params.customDeals[i].proposal.start_epoch);
            buf.writeChainEpoch(params.customDeals[i].proposal.end_epoch);
            buf.writeBytes(params.customDeals[i].proposal.storage_price_per_epoch.serializeBigInt());
            buf.writeBytes(params.customDeals[i].proposal.provider_collateral.serializeBigInt());
            buf.writeBytes(params.customDeals[i].proposal.client_collateral.serializeBigInt());
            buf.writeChainEpoch(params.customDeals[i].proposal.CustomOptions.termination_epoch);
            buf.writeUInt64(params.customDeals[i].proposal.CustomOptions.share_percentage);
            buf.writeBool(params.customDeals[i].proposal.CustomOptions.compute_req);

            buf.writeBytes(params.customDeals[i].client_signature);
        }
    ...
    function deserializePublishCustomStorageDealsReturn(bytes memory rawResp) internal pure returns (MarketTypes.PublishCustomStorageDealsReturn memory ret) {
        uint byteIdx = 0;
        uint len;
    ...
    function serializeCustomOptions(MarketTypes.CustomOptions memory params) internal pure returns (uint capacity, bytes memory) {
        uint capacity = 0

        capacity += Misc.getPrefixSize(3)
        capacity += Misc.getChainEpochSize(params.termination_epoch)
        capacity += Misc.getPrefixSize(params.share_percentage)
        capacity += Misc.getBoolSize()

        CBOR.CBORBuffer memory buf = CBOR.create(capacity);

        buf.writeChainEpoch(params.termination_epoch);
        buf.writeUInt64(params.share_percentage)
        buf.writeBool(params.compute_req);

        return capacity, buf.data;
    ...
    function deserializeMarketCustomDealNotifyParams(bytes memory rawResp) internal pure returns (MarketTypes.MarketCustomDealNotifyParams memory ret) {
        uint byteIdx = 0;
        uint len;
    ...
    function serializeCustomDealProposal(MarketTypes.CustomDealProposal memory customProposal) internal pure returns (bytes memory) {
        uint256 capacity = 0;
        bytes memory storage_price_per_epoch = customProposal.storage_price_per_epoch.serializeBigInt();
        bytes memory provider_collateral = customProposal.provider_collateral.serializeBigInt();
        bytes memory client_collateral = customProposal.client_collateral.serializeBigInt();

        capacity += Misc.getPrefixSize(14);
        capacity += Misc.getCidSize(customProposal.piece_cid.data);
        capacity += Misc.getPrefixSize(customProposal.piece_size);
        capacity += Misc.getBoolSize();
        capacity += Misc.getBytesSize(customProposal.client.data);
        capacity += Misc.getBytesSize(customProposal.provider.data);
        capacity += Misc.getBytesSize(customProposal.label.data);
        capacity += Misc.getChainEpochSize(customProposal.start_epoch);
        capacity += Misc.getChainEpochSize(customProposal.end_epoch);
        capacity += Misc.getBytesSize(storage_price_per_epoch);
        capacity += Misc.getBytesSize(provider_collateral);
        capacity += Misc.getBytesSize(client_collateral);
        capacity += Misc.getChainEpochSize(customProposal.CustomOptions.termination_epoch);
        capacity += Misc.getPrefixSize(customProposal.CustomOptions.share_percentage);
        capacity += Misc.getBoolSize(customProposal.CustomOptions.compute_req);
        CBOR.CBORBuffer memory buf = CBOR.create(capacity);

        buf.startFixedArray(14);

        buf.writeCid(customProposal.piece_cid.data);
        buf.writeUInt64(customProposal.piece_size);
        buf.writeBool(customProposal.verified_deal);
        buf.writeBytes(customProposal.client.data);
        buf.writeBytes(customProposal.provider.data);
        buf.writeDealLabel(customProposal.label);
        buf.writeChainEpoch(customProposal.start_epoch);
        buf.writeChainEpoch(customProposal.end_epoch);
        buf.writeBytes(storage_price_per_epoch);
        buf.writeBytes(provider_collateral);
        buf.writeBytes(client_collateral);
        buf.writeChainEpoch(customProposal.CustomOptions.termination_epoch);
        buf.writeUInt64(customProposal.CustomOptions.share_percentage);
        buf.writeBool(customProposal.CustomOptions.compute_req);

        return buf.data();
    }

    function deserializeCustomDealProposal(bytes memory rawResp) internal pure returns (MarketTypes.CustomDealProposal memory ret) {
        uint byteIdx = 0;
        uint len;
        bytes memory tmp;
        ...
        (ret.CustomOptions.termination_epoch, byteIdx) = rawResp.readChainEpoch(byteIdx);
        (ret.CustomOptions.share_percentage, byteIdx) = rawResp.readUInt64(byteIdx);
        (ret.CustomOptions.compute_req, byteIdx) = rawResp.readBool(byteIdx);
